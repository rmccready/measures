...
(function() {
 ...
 root.dtap_numerator = function (measure, birthdate, effective_date){
    ...
    var number_dtap_vaccine_administered = inRange(measure.dtap_vaccine_medication_administered,
                                              earliest_vaccine,
                                              latest_vaccine);
    var number_dtap_vaccine_procedure = inRange(measure.dtap_vaccination_procedure_performed,
                                              earliest_vaccine,
                                              latest_vaccine);
  ...
  root.dtap_exclusion = function(measure, birthdate, effective_date){
    many_exclusions = _.flatten(_.compact(new Array(measure.dtap_vaccine_medication_allergy,
...
/// IPV -- numerator 2
 root.ipv_numerator = function (measure, birthdate, effective_date){
    ...
    var earliest_vaccine = birthdate + 42 * day;
    var latest_vaccine =   birthdate + 2  * year;
    var number_ipv_vaccine_administered = inRange(measure.ipv_vaccine_medication_administered,
                                              earliest_vaccine,
                                              latest_vaccine);
    var number_ipv_vaccine_procedure =  inRange(measure.ipv_vaccination_procedure_performed,
                                              earliest_vaccine,
                                              latest_vaccine);
  ...
  root.ipv_exclusion = function(measure, birthdate, effective_date){ 
    var many_exclusions = _.flatten(_.compact(new Array(measure.ipv_medication_allergy,
...
/// MMR  Numerator 3
 root.mmr_numerator = function (measure,  birthdate, effective_date){
    ...
    var number_mmr_vaccine_administered =     inRange(measure.mmr_vaccine_medication_administered,
                                                  earliest_vaccine,
                                                  latest_vaccine);
    var number_measles_vaccine_administered = inRange(measure.measles_vaccine_medication_administered,
                                                  earliest_vaccine,
                                                  latest_vaccine);
    var number_mumps_vaccine_administered =   inRange(measure.mumps_vaccine_medication_administered,
                                                   earliest_vaccine,
                                                  latest_vaccine);
    var number_rubella_vaccine_administered = inRange(measure.rubella_vaccine_medication_administered,
                                                  earliest_vaccine,
                                                  latest_vaccine);
    var number_mmr_vaccine_procedure =     inRange(measure.mmr_vaccine_procedure_performed,
                                                  earliest_vaccine,
                                                  latest_vaccine);
    var number_measles_vaccine_procedure = inRange(measure.measles_vaccine_procedure_performed,
                                                  earliest_vaccine,
                                                  latest_vaccine);
    var number_mumps_vaccine_procedure =   inRange(measure.mumps_vaccine_procedure_performed,
                                                   earliest_vaccine,
                                                  latest_vaccine);
    var number_rubella_vaccine_procedure = inRange(measure.rubella_vaccine_procedure_performed,
                                                  earliest_vaccine,
                                                  latest_vaccine);
...
// HiB - Numerator 4
 root.hib_numerator = function (measure, birthdate, effective_date){
   ...
   var earliest_vaccine = birthdate + 42 * day;
   var latest_vaccine =   birthdate + 2  * year;
    var number_hib_vaccine_administered = inRange(measure.hib_vaccine_medication_administered,
                                              earliest_vaccine,
                                              latest_vaccine);
    var number_hib_vaccine_procedure =  inRange(measure.hib_vaccination_procedure_performed,
                                              earliest_vaccine,
                                              latest_vaccine);
...
// Hepatitis B -- Numerator 5
   root.hep_b_numerator = function (measure, birthdate, effective_date){
    ...
    var earliest_vaccine = birthdate;
    var latest_vaccine = birthdate + 2 * year;
    var number_hep_b_vaccine_procedure = inRange(measure.hepatitis_b_vaccination_procedure_performed,
                                              earliest_vaccine,
                                              latest_vaccine);
    var number_hep_b_vaccine_administered = inRange(measure.hepatitis_b_vaccine_medication_administered,
                                              earliest_vaccine,
                                              latest_vaccine);

    return (number_hep_b_vaccine_administered >= 3   ||  number_hep_b_vaccine_procedure >= 3 ||
             (conditionResolved(measure.hepatitis_b_diagnosis_diagnosis_resolved, birthdate, effective_date)));
  };

  // Exclude patients who have an allergy to hep_b Vaccine
  root.hep_b_exclusion = function(measure, birthdate, effective_date){ 
    return ( (inRange(measure.hepatitis_b_vaccine_medication_allergy, birthdate, effective_date))  ||
             (inRange(measure.baker_s_yeast_substance_substance_allergy, birthdate, effective_date)) ||
             (inRange(measure.baker_s_yeast_medication_allergy, birthdate, effective_date)) );
  };
...
root.vzv_numerator = function (measure, birthdate, effective_date){
    ...
    var number_vzv_vaccine_administered = inRange(measure.vzv_vaccine_medication_administered,
                                              earliest_vaccine,
                                              latest_vaccine);
    var number_vzv_vaccine_procedure =    inRange(measure.vzv_vaccination_procedure_performed,
                                              earliest_vaccine,
                                              latest_vaccine);

     return ( (number_vzv_vaccine_administered >= 1 || number_vzv_vaccine_procedure >= 1) ||
              (conditionResolved(measure.vzv_diagnosis_resolved, birthdate, effective_date)) );
   };
...
root.pcv_numerator = function (measure,  birthdate, effective_date){
   ...
   var latest_vaccine = birthdate + 2 * year;
   var earliest_vaccine = birthdate;
   var number_pcv_vaccine_administered = inRange(measure.pneumococcal_vaccine_medication_administered,
                                              earliest_vaccine,
                                              latest_vaccine);
     var number_pcv_vaccine_procedure =    inRange(measure.pneumococcal_vaccination_medication_administered,
                                              earliest_vaccine,
                                              latest_vaccine);
...
// Hepatitis A -- numerator 8
   ...
   var latest_vaccine = birthdate + 2 * year;
   var earliest_vaccine = birthdate;
    var number_hep_a_vaccine_administered = inRange(measure.hepatitis_a_vaccination_procedure_performed,
                                              earliest_vaccine,
                                              latest_vaccine);
    var number_hep_a_vaccine_procedure =    inRange(measure.hepatitis_a_vaccine_medication_administered,
                                              earliest_vaccine,
                                              latest_vaccine);

    return (number_hep_a_vaccine_administered >= 2   || number_hep_a_vaccine_procedure >= 2 ||
             (conditionResolved(measure.hepatitis_a_diagnosis_diagnosis_resolved, birthdate, effective_date)));
  };
...
/// Rotavirus -- numerator 9
 ...
 root.rv_numerator = function (measure, birthdate, effective_date){
  ...
  // the patients' birthdate
   var latest_vaccine = birthdate + 2 * year;
   var earliest_vaccine = birthdate;
    var number_rv_vaccine_administered = inRange(measure.rotavirus_vaccine_medication_administered,
                                              earliest_vaccine,
                                              latest_vaccine);
    var number_rv_vaccine_procedure =  inRange(measure.rotavirus_vaccination_procedure_performed,
                                              earliest_vaccine,
                                              latest_vaccine);
