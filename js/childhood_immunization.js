...
(function () {
  ...
  root.dtap_numerator = function (measure, birthdate, effective_date) {
    ...
    var number_dtap_vaccine_administered = inRange(measure.dtap_vaccine_medication_administered, earliest_vaccine, latest_vaccine);
    var number_dtap_vaccine_procedure = inRange(measure.dtap_vaccination_procedure_performed, earliest_vaccine, latest_vaccine);
  ...
  root.dtap_exclusion = function (measure, birthdate, effective_date) {
    many_exclusions = _.flatten(_.compact(new Array(measure.dtap_vaccine_medication_allergy, measure.encephalopathy_diagnosis_active, measure.progressive_neurologic_disorder_diagnosis_active)));
  ...
  root.ipv_numerator = function (measure, birthdate, effective_date) {
    ...
    var earliest_vaccine = birthdate + 42 * day;
    var latest_vaccine = birthdate + 2 * year;
    var number_ipv_vaccine_administered = inRange(measure.ipv_vaccine_medication_administered, earliest_vaccine, latest_vaccine);
    var number_ipv_vaccine_procedure = inRange(measure.ipv_vaccination_procedure_performed, earliest_vaccine, latest_vaccine);

    return (number_ipv_vaccine_administered >= 3 || number_ipv_vaccine_procedure >= 3);
  };

  // Exclude patients who have an allergy to ipv Vaccine and various medications
  root.ipv_exclusion = function (measure, birthdate, effective_date) {
    var many_exclusions = _.flatten(_.compact(new Array(measure.ipv_medication_allergy, measure.neomycin_medication_allergy, measure.streptomycin_medication_allergy, measure.polymyxin_medication_allergy)));
    return (inRange(many_exclusions, birthdate, effective_date));
  ...
  root.mmr_numerator = function (measure, birthdate, effective_date) {
    ...
    var number_mmr_vaccine_administered = inRange(measure.mmr_vaccine_medication_administered, earliest_vaccine, latest_vaccine);
    var number_measles_vaccine_administered = inRange(measure.measles_vaccine_medication_administered, earliest_vaccine, latest_vaccine);
    var number_mumps_vaccine_administered = inRange(measure.mumps_vaccine_medication_administered, earliest_vaccine, latest_vaccine);
    var number_rubella_vaccine_administered = inRange(measure.rubella_vaccine_medication_administered, earliest_vaccine, latest_vaccine);
    var number_mmr_vaccine_procedure = inRange(measure.mmr_vaccine_procedure_performed, earliest_vaccine, latest_vaccine);
    var number_measles_vaccine_procedure = inRange(measure.measles_vaccine_procedure_performed, earliest_vaccine, latest_vaccine);
    var number_mumps_vaccine_procedure = inRange(measure.mumps_vaccine_procedure_performed, earliest_vaccine, latest_vaccine);
    var number_rubella_vaccine_procedure = inRange(measure.rubella_vaccine_procedure_performed, earliest_vaccine, latest_vaccine);
  ...
  root.mmr_exclusion = function (measure, birthdate, effective_date) {
    many_exclusions = _.flatten(_.compact(new Array(measure.cancer_of_lymphoreticular_or_histiocytic_tissue_diagnosis_active, measure.cancer_of_lymphoreticular_or_histiocytic_tissue_diagnosis_inactive, measure.hiv_disease_diagnosis_active, measure.multiple_myeloma_diagnosis_active, measure.leukemia_diagnosis_active, measure.immunodeficiency_diagnosis_active, measure.measles_vaccine_medication_allergy, measure.mumps_vaccine_medication_allergy, measure.rubella_vaccine_medication_allergy)));
    return (inRange(many_exclusions, birthdate, effective_date));
  ...
  root.hib_numerator = function (measure, birthdate, effective_date) {
    ...
    var earliest_vaccine = birthdate + 42 * day;
    var latest_vaccine = birthdate + 2 * year;
    var number_hib_vaccine_administered = inRange(measure.hib_vaccine_medication_administered, earliest_vaccine, latest_vaccine);
    var number_hib_vaccine_procedure = inRange(measure.hib_vaccination_procedure_performed, earliest_vaccine, latest_vaccine);
  ...
  root.hep_b_numerator = function (measure, birthdate, effective_date) {
    ...
    var earliest_vaccine = birthdate;
    var latest_vaccine = birthdate + 2 * year;
    var number_hep_b_vaccine_procedure = inRange(measure.hepatitis_b_vaccination_procedure_performed, earliest_vaccine, latest_vaccine);
    var number_hep_b_vaccine_administered = inRange(measure.hepatitis_b_vaccine_medication_administered, earliest_vaccine, latest_vaccine);
  ...
  // Exclude patients who have an allergy to hep_b Vaccine
  ...
  root.vzv_numerator = function (measure, birthdate, effective_date) {
    ...
    var number_vzv_vaccine_administered = inRange(measure.vzv_vaccine_medication_administered, earliest_vaccine, latest_vaccine);
    var number_vzv_vaccine_procedure = inRange(measure.vzv_vaccination_procedure_performed, earliest_vaccine, latest_vaccine);
  ...
  root.pcv_numerator = function (measure, birthdate, effective_date) {
    ...
    var latest_vaccine = birthdate + 2 * year;
    var earliest_vaccine = birthdate;
    var number_pcv_vaccine_administered = inRange(measure.pneumococcal_vaccine_medication_administered, earliest_vaccine, latest_vaccine);
    var number_pcv_vaccine_procedure = inRange(measure.pneumococcal_vaccination_medication_administered, earliest_vaccine, latest_vaccine);
  ...
  root.hep_a_numerator = function (measure, birthdate, effective_date) {
    ...
    var latest_vaccine = birthdate + 2 * year;
    var earliest_vaccine = birthdate;
    var number_hep_a_vaccine_administered = inRange(measure.hepatitis_a_vaccination_procedure_performed, earliest_vaccine, latest_vaccine);
    var number_hep_a_vaccine_procedure = inRange(measure.hepatitis_a_vaccine_medication_administered, earliest_vaccine, latest_vaccine);
  ...
  root.rv_numerator = function (measure, birthdate, effective_date) {
    ...
    var latest_vaccine = birthdate + 2 * year;
    var earliest_vaccine = birthdate;
    var number_rv_vaccine_administered = inRange(measure.rotavirus_vaccine_medication_administered, earliest_vaccine, latest_vaccine);
    var number_rv_vaccine_procedure = inRange(measure.rotavirus_vaccination_procedure_performed, earliest_vaccine, latest_vaccine);

    return (number_rv_vaccine_administered >= 2 || number_rv_vaccine_procedure >= 2);
  };

  // Exclude patients who have an allergy to rv Vaccine
  ...
  root.inf_numerator = function (measure, birthdate, effective_date) {
    ...
    var latest_vaccine = birthdate + 2 * year;
    var number_inf_vaccine_administered = inRange(measure.influenza_vaccination_procedure_performed, earliest_vaccine, latest_vaccine) + inRange(measure.influenza_vaccine_medication_administered, earliest_vaccine, latest_vaccine);
  ...
  root.unique_dates = function (times) {
    ...
    dates = _.map(times, function (time) {
