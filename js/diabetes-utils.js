...
(function() {
  ...
  root.diabetes_denominator = function(measure, earliest_diagnosis, effective_date) {
    ...
    encounter_ophthalmology =       inRange(measure.encounter_ophthalmology,       earliest_diagnosis, effective_date);

    return (has_medications_indicative_of_diabetes(measure, earliest_diagnosis, effective_date)
            ||
            (diagnosis_diabetes && ((encounter_acute_inpatient || encounter_ed)
                                    ||
                                    (encounter_non_acute_inpatient >= 2
                                     || encounter_outpatient       >= 2
                                     || encounter_ophthalmology    >= 2))));
  }
  ...
  root.diabetes_exclusions = function(measure, earliest_diagnosis, effective_date) {
    ...
    return ((measure.polycystic_ovaries
             &&
             !(diagnosis_diabetes
               && (encounter_acute_inpatient
                   || encounter_ed
                   || encounter_non_acute_inpatient
                   || encounter_outpatient
                   || encounter_ophthalmology)))
            ||
            ((diagnosis_gestational_diabetes || diagnosis_steroid_induced_diabetes) 
             ...
             && !(diagnosis_diabetes && (encounter_acute_inpatient
                                         || encounter_ed
                                         || encounter_non_acute_inpatient
                                         || encounter_outpatient
                                         || encounter_ophthalmology))));
