...
#    STDERR.puts JSON.pretty_generate(property_json)
    ...
    property_json.each_pair do |key,value|
     ...
     report[concept]['conceptID'] = conceptID
     ...
     report[concept]['concept_ids'].add(value["standard_concept_id"])
     report[concept]['standard_category'] = value["standard_category"]
     # STDERR.puts "concept_ID= #{conceptID}   measures =  #{report[conceptID]['measures'].to_a}   qds = #{report[conceptID]['qds_data_types'].to_a}"
    end
